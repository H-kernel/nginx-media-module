...
/* begin add by H.kernel for state the video file size */
#include "libMediaKenerl.h"
#include "mk_def.h"
/* end add by H.kernel for state the video file size */
/* begin add by H.kernl for upload report */
#include <curl/curl.h>
#include <common/as_json.h>
/* end add by H.kernl for upload report */
...
typedef struct ngx_media_upload_ctx_s {
    ...
    /* begin add by H.kernel for stat video file */
    ngx_uint_t                          video_duration;
    /* end add by H.kernel for stat video file */
    ...
    /* begin add by H.kernel for stat video file */
    unsigned int                        stat_video_file:1;
    /* end add by H.kernel for stat video file */
    unsigned int                        ignore_client_abort:1;
    /* begin add by H.kernel for report by curl */
    cJSON                              *root;
    ngx_str_t                           json;
    /* end add by H.kernel for report by curl */
} ngx_media_upload_ctx_t;

static ngx_int_t ngx_media_upload_test_expect(ngx_http_request_t *r);

#if (NGX_HTTP_V2)
static void      ngx_media_upload_read_event_handler(ngx_http_request_t *r);
#endif
...
static ngx_int_t ngx_media_upload_video_duration_variable(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_int_t ngx_media_upload_process_field_templates(ngx_http_request_t *r, ngx_media_upload_field_template_t *t, ngx_str_t *field_name, ngx_str_t *field_value);
...
/* begin add by H.kernel for stat video file */
static void      ngx_media_upload_stat_video_file(ngx_media_upload_ctx_t *u);
/* end add by H.kernel for stat video file */
...
static ngx_int_t ngx_media_upload_append_field_josn(ngx_media_upload_ctx_t *u,ngx_str_t *name, ngx_str_t *value);
...
/* begin add by H.kernl for upload report */
static ngx_int_t ngx_media_upload_report(ngx_str_t* url,ngx_str_t* args,ngx_http_request_t *r,ngx_media_upload_ctx_t *ctx);
/* end add by H.kernl for upload report */
...
static ngx_command_t  ngx_media_upload_commands[] = { /* {{{ */
    ...
    /*
     * begin add by H.kernel  
     * Specifies state the video file
     */
    { ngx_string("upload_stat_video_info"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                         |NGX_CONF_FLAG,
       ngx_conf_set_flag_slot,
       NGX_HTTP_LOC_CONF_OFFSET,
       offsetof(ngx_media_upload_loc_conf_t, stat_video_file),
       NULL },
    
    /* 
     * add by H.kernel
     * Specifies for upload ignore client abort
     */
    { ngx_string("upload_ignore_client_abort"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_HTTP_LIF_CONF
                         |NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_media_upload_loc_conf_t, ignore_client_abort),
      NULL },
...
static ngx_http_variable_t  ngx_media_upload_aggregate_variables[] = { /* {{{ */
    ...
    /* begin add by H.kernel for stat video file */
    { ngx_string("upload_video_duration"), NULL, ngx_media_upload_video_duration_variable,
      (uintptr_t) offsetof(ngx_media_upload_ctx_t, video_duration),
      NGX_HTTP_VAR_CHANGEABLE|NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },
...
ngx_media_upload_handler(ngx_http_request_t *r)
{
    ...
    /* add by H.kernel for stat video file */
    u->stat_video_file = ulcf->stat_video_file;
    u->ignore_client_abort = ulcf->ignore_client_abort;
    u->video_duration  = 0;
    ...
    if (r->stream) {
        ...
        rc = ngx_http_read_client_request_body(r, ngx_media_upload_read_event_handler);
...
#if (NGX_HTTP_V2)
static void
ngx_media_upload_read_event_handler(ngx_http_request_t *r)
{
    ...
    r->read_event_handler = ngx_media_upload_read_event_handler;
...
static ngx_int_t ngx_media_upload_add_headers(ngx_http_request_t *r, ngx_media_upload_loc_conf_t *ulcf) { /* {{{ */
    ...
    if(ulcf->header_templates != NULL) {
        ...
        for (i = 0; i < ulcf->header_templates->nelts; i++) {
            if (ngx_media_upload_process_field_templates(r, &t[i], &name, &value) != NGX_OK) {
                return NGX_ERROR;
            }
...
static ngx_int_t ngx_media_upload_body_handler(ngx_http_request_t *r) { /* {{{ */
    ...
    if(ctx->no_content) {
        ...
        rc = ngx_media_upload_append_field_josn(ctx, &dummy, &ngx_media_upload_empty_field_value);
    ...
    *b->last++ = LF;
    ...
    if(7 < uri.len && (ngx_strncmp(uri.data, "http://", 7) == 0)) {
        rc = ngx_media_upload_report(&uri,&args,r,ctx);
    }
    else if(uri.len != 0 && uri.data[0] == '/') {
        rc = ngx_http_internal_redirect(r, &uri, &args);
        /* reset the ctx to the request */
        ngx_http_set_ctx(r, ctx, ngx_media_upload_module);
        
        ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,"http internal redirect to :%V,return:%d",&uri,rc);
        
    }
    else{
        rc = ngx_http_named_location(r, &uri);
        ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,"http named loacation to :%V,return:%d",&uri,rc);
    }

    if (rc == NGX_ERROR) {
        ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                          "http internal redirect or named loaction return:%d",
                          rc);
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
...
static ngx_int_t
ngx_media_upload_process_field_templates(
    ngx_http_request_t *r, ngx_media_upload_field_template_t *t,
    ...
    if(u->is_file) {
        ...
        if(ulcf->field_templates) {
            ...
            for (i = 0; i < ulcf->field_templates->nelts; i++) {
                rc = ngx_media_upload_process_field_templates(r, &t[i], &field_name, &field_value);
                ...
                rc = ngx_media_upload_append_field_josn(u, &field_name, &field_value);
        ...
        if(pass_field && u->field_name.len != 0) {
            ...
            rc = ngx_media_upload_append_field_josn(u, &u->field_name, &ngx_media_upload_empty_field_value);
...
static void ngx_media_upload_finish_handler(ngx_media_upload_ctx_t *u) { /* {{{ */
    ...
    if(u->is_file) {
        ...
        /* begin add by H.kernel for stat video file */
        if(u->stat_video_file) {
            ngx_media_upload_stat_video_file(u);
        }
        ...
        if(ulcf->aggregate_field_templates) {
            ...
            for (i = 0; i < ulcf->aggregate_field_templates->nelts; i++) {
                rc = ngx_media_upload_process_field_templates(r, &af[i], &aggregate_field_name,
                ...
                rc = ngx_media_upload_append_field_josn(u, &aggregate_field_name, &aggregate_field_value);
...
/* begin add by H.kernel for stat video file */
static void      
ngx_media_upload_stat_video_file(ngx_media_upload_ctx_t *u)
{
    u->video_duration = mk_stat_media_file_duration((char*)u->output_file.name.data);
    ngx_log_error(NGX_LOG_INFO, u->log, 0,
                           "stat the to video file \"%V\" duration:%ui", &u->output_file.name,u->video_duration);
    return;
}
/* end add by H.kernel for stat video file */
static ngx_int_t 
ngx_media_upload_flush_output_buffer(ngx_media_upload_ctx_t *u, u_char *buf, size_t len) { /* {{{ */
    ngx_http_request_t               *r = u->request;
    ngx_buf_t                        *b;
    ngx_chain_t                      *cl;
    ngx_media_upload_loc_conf_t *ulcf = ngx_http_get_module_loc_conf(r, ngx_media_upload_module);
...
static ngx_int_t 
ngx_media_upload_append_field_josn(ngx_media_upload_ctx_t *u,ngx_str_t *name, ngx_str_t *value)
{
    char *jName,*jValue;
    u_char *last;

    if(name->len > 0) {
        jName = ngx_palloc(u->request->pool, name->len + 1);
        if(NULL == jName) {
            return NGX_OK;
        }
        last = ngx_copy(jName,name->data,name->len);
        *last = '\0';

        jValue = ngx_palloc(u->request->pool, value->len + 1);
        if(NULL == jValue) {
            return NGX_OK;
        }
        last = ngx_copy(jValue,value->data,value->len);
        *last = '\0';

        cJSON_AddItemToObject(u->root,jName,cJSON_CreateString(jValue));
    }
...
ngx_media_upload_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->stat_video_file = NGX_CONF_UNSET;
...
ngx_media_upload_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if(conf->stat_video_file == NGX_CONF_UNSET) {
        conf->stat_video_file = (prev->stat_video_file != NGX_CONF_UNSET) ?
            prev->stat_video_file : 0;
    }

    if(conf->ignore_client_abort == NGX_CONF_UNSET) {
        conf->ignore_client_abort = (prev->ignore_client_abort != NGX_CONF_UNSET) ?
...
static ngx_int_t
ngx_media_upload_hash_variable(ngx_http_request_t *r,
    ...
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ...
    return ngx_media_upload_hash_variable(r, v, data, u->md5_ctx->md5_digest, MD5_DIGEST_LENGTH);
} /* }}} */
    ...
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ...
    return ngx_media_upload_hash_variable(r, v, data, u->sha256_ctx->sha256_digest, SHA256_DIGEST_LENGTH);
} /* }}} */
    ...
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ...
    return ngx_media_upload_hash_variable(r, v, data, u->sha512_ctx->sha512_digest, SHA512_DIGEST_LENGTH);
} /* }}} */
...
/* begin add by H.kernel for stat video file */
static ngx_int_t /* {{{ ngx_media_upload_video_duration_variable */
ngx_media_upload_video_duration_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v,  uintptr_t data)
{
    ngx_media_upload_ctx_t  *u;
    u_char                 *p;
...
ngx_http_do_read_upload_client_request_body(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        for ( ;; ) {
            ...
            if (n == 0) {
                if((NULL != u )&& (!u->ignore_client_abort)) {
                    ngx_log_error(NGX_LOG_INFO, c->log, 0,
                              "upload client closed prematurely connection,ignore[%d].",u->ignore_client_abort);
                   c->error = 1;
                   return NGX_HTTP_BAD_REQUEST;
                }
                ngx_log_error(NGX_LOG_INFO, c->log, 0,
                              "upload client closed prematurely connection,ignore client abort on");                
            }
            else if (n == NGX_ERROR) {
                c->error = 1;
                return NGX_HTTP_BAD_REQUEST;
...
static void upload_init_ctx(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
    ...
    /* begin add by H.kernel for report by curl */
    upload_ctx->root      = cJSON_CreateObject();
    upload_ctx->json.data = NULL;
    upload_ctx->json.len  = 0;
    /* end add by H.kernel for report by curl */
} /* }}} */
...
static void upload_shutdown_ctx(ngx_media_upload_ctx_t *upload_ctx) { /* {{{ */
	if(upload_ctx != 0) {
        ...
        /* begin add by H.kernel for report by curl */
        if(NULL != upload_ctx->root) {
            char * json_buf = (char*)cJSON_PrintUnformatted(upload_ctx->root);
            ngx_uint_t len        = ngx_strlen(json_buf);
            upload_ctx->json.data = ngx_palloc(upload_ctx->request->pool, len + 1);
            if(NULL != upload_ctx->json.data) {
                u_char* last = ngx_cpymem(upload_ctx->json.data,json_buf,len);
                *last = '\0';
                upload_ctx->json.len = len + 1;
            }
            free(json_buf);
            cJSON_Delete(upload_ctx->root);
            upload_ctx->root = NULL;
        }
        /* end add by H.kernel for report by curl */
	}
...
static ngx_int_t upload_process_buf(ngx_media_upload_ctx_t *upload_ctx, u_char *start, u_char *end) { /* {{{ */
	...
	if(start == end) {
		if(upload_ctx->state != upload_state_finish) {
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_CORE, upload_ctx->log, 0,
                           "Otherwise confirm end of stream ,statue:[%d].",upload_ctx->state);
...
/* begin add by H.kernl for upload report */
static size_t
ngx_media_upload_curl_callback(char *ptr, size_t size, size_t nmemb, void *userdata)
{
    ngx_chain_t* out = NULL;
    ngx_buf_t*  buf  = NULL;
    u_char*     data = NULL;
    ngx_media_upload_curl_req_t* req = (ngx_media_upload_curl_req_t*)userdata;
    if(NULL == req) {
        return NGX_ERROR;
    }
    ...
    do {
        out = ngx_pcalloc(req->r->pool, sizeof(ngx_chain_t));
        if(NULL == out){
            break;
        }
        buf = ngx_pcalloc(req->r->pool, sizeof(ngx_buf_t));
        if(NULL == buf){
            break;
        }
        data = ngx_pcalloc(req->r->pool, recv_size);
        if(NULL == data) {
            break;
        }
        ngx_memcpy(data, ptr, recv_size);
        buf->pos = data;
        buf->last = data + recv_size;
...
static ngx_int_t 
ngx_media_upload_report(ngx_str_t* url,ngx_str_t* args,ngx_http_request_t *r,ngx_media_upload_ctx_t *ctx)
{
    CURL                           *req;
    ...
    u_char                         *strUrl = NULL;
    u_char                         *last   = NULL;
    ngx_media_upload_curl_req_t     curl_req;

    strUrl = ngx_pcalloc(r->pool, url->len +args->len + 2);
    if(NULL == strUrl) {
        return NGX_ERROR;
    }
    last = ngx_cpymem(strUrl,url->data,url->len);
    if(0 < args->len){
        *last ='?';last++;
        last = ngx_cpymem(last,args->data,args->len);
    }
    *last = '\0';
    ...
    if(NULL == req) {
        ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                              "ngx_media_upload_report, url:[%s] curl init fail.",
                              strUrl);
        return NGX_ERROR;
    }
	...
	headList = curl_slist_append(NULL, "Content-Type:application/json");
	curl_easy_setopt(req, CURLOPT_HTTPHEADER, headList);

	curl_easy_setopt(req, CURLOPT_WRITEFUNCTION, ngx_media_upload_curl_callback);
	curl_easy_setopt(req, CURLOPT_WRITEDATA, &curl_req);

    curl_easy_setopt(req, CURLOPT_URL, strUrl);
    if(NULL !=  ctx->json.data) {
        curl_easy_setopt(req, CURLOPT_POSTFIELDS, (char*)ctx->json.data);
    }
    
    if(NULL != ctx->json.data) {
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                              "ngx_media_upload_report, uri:[%s] ,request:\n%s.",
                              strUrl,ctx->json.data);
    }
    else {
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                              "ngx_media_upload_report, uri:[%s]",strUrl);
    }
    ret = NGX_OK;
    do
    {
    	...
    	if (NGX_HTTP_OK != nResponseCode) {
            ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                              "ngx_media_upload_report, url:[%s] HTTP response is not 200 OK,code:[%d].",
                              strUrl,nResponseCode);
            ret = NGX_ERROR;
    		break;
    	}
    ...
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                              "ngx_media_upload_report, url:[%s] end",strUrl);
    ...
    if(NGX_OK == ret) {
        ...
        if(NULL == curl_req.first) {
            ngx_http_finalize_request(r, NGX_DONE);
        }
        else {
            curl_req.last->buf->last_buf = 1;
            ngx_http_finalize_request(r, ngx_http_output_filter(r, curl_req.first));
        }
    }
    return ret;
}
