...
static ngx_command_t ngx_http_flv_live_commands[] = {
    ...
    /* begin add by H.Kernel for default rtmp port and APP */
    ...
    /* end add by H.Kernel for default rtmp port and APP */
    ...
    ngx_rtmp_connection_t *rconn)
{
    ...
    if (ngx_http_arg(r, arg_port.data, arg_port.len, &ctx->port) != NGX_OK) {
        ...
        if((NULL != hfcf->default_port.data)&&(0 < hfcf->default_port.len)) {
            ctx->port.data = hfcf->default_port.data;
            ctx->port.len  = hfcf->default_port.len;
    ...
    if (ngx_http_arg(r, arg_stream.data, arg_stream.len, 
                     &ctx->stream) != NGX_OK) 
	{
        /* use the stream with request uri */
        if (r->args_start)
        {
            len  = r->args_start - 1 - r->uri_start;
            last = &r->uri.data[len];
        }
        else {
            len  = r->uri.len;
        ...
        start = ngx_video_strrchr(r->uri.data,last,'/');
        if(NULL == start) {
            return NGX_ERROR;
        }
        start++; /* skip th '/' */
        len = last - start;
        ctx->stream.len  = len;
	    ctx->stream.data = ngx_pcalloc(r->pool, len + 1);
        last = ngx_cpymem(ctx->stream.data,start,len);
        *last = '\0';
        
    }
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                      "flv live: request rtmp port:[%V] app:[%V] stream:[%V]",
                      &ctx->port,&ctx->app,&ctx->stream);
...
ngx_http_flv_live_meta_message(ngx_rtmp_session_t *s, ngx_chain_t *in)
{
    ...
    ngx_memzero(&ch,sizeof(ngx_rtmp_header_t));
    ch.timestamp = 0;
