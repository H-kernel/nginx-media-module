...
ngx_http_client_free_request(ngx_http_request_t *hcr)
{
    ...
    if (ctx->in) {
        ngx_free_chain(hcr->pool,ctx->in);
        ctx->in = NULL;
    }
...
ngx_http_client_set_url(ngx_http_request_t *r, ngx_str_t *url, ngx_log_t *log)
{
    ...
    if (ngx_parse_url(r->pool,&ctx->url) != NGX_OK) {
         if (ctx->url.err) {
            ngx_log_error(NGX_LOG_ERR, log, 0,
                          "%s in http client \"%V\"", ctx->url.err, &ctx->url.url);
        }
        return NGX_ERROR;
    }
...
static ngx_http_client_header_out_t*
ngx_http_client_find_header(ngx_http_client_ctx_t *ctx,ngx_str_t *header)
{
    ...
    for (i = 0; i < ctx->headers_out.headers.nelts; ++i, ++h) {
        if (h->key.len != header->len) {
            continue;
        }

        if(0 != ngx_memcmp(h->key.data,header->data,header->len)) {
            continue;
        }
        ...
        ngx_str_t *value)
{
    ...
    h = ngx_http_client_find_header(ctx,header);
    if (NULL == h) {// header not exist
...
ngx_http_client_create_request_buf(ngx_client_session_t *s)
{
    ...
    *b->last++ = '/';
    if (ctx->url.uri.len) {
        b->last = ngx_cpymem(b->last, ctx->url.uri.data, ctx->url.uri.len);
    }
...
ngx_http_client_body_chunked(ngx_http_request_t *r, ngx_chain_t *cl)
{
    ...
    while (1) {
        ...
        if (rc == NGX_OK) {
            ...
            while (1) {
                if (*ll == NULL) {
                    bufs.size = hccf->body_buffer_size;
                    bufs.num  = 1;
                    *ll = ngx_create_chain_of_bufs(r->pool, &bufs);
                    if (*ll == NULL) {
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
...
ngx_http_client_read_body(ngx_http_request_t *r, ngx_chain_t **in)
{
    ...
    if (ctx->buffer->last != ctx->buffer->pos) {
        bufs.size = hccf->body_buffer_size;
        bufs.num  = 1;
        ln = ngx_create_chain_of_bufs(r->pool, &bufs);
        if (ln == NULL) {
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
