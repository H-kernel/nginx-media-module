...
static unsigned int ngx_http_zmq_to_ms(clock_t delta) {return (int)((float)delta / CLOCKS_PER_SEC * 1000);}

static int ngx_http_zmq_get_socktype(ngx_http_request_t *r);
...
ngx_http_zmq_create_loc_conf(ngx_conf_t *cf)
{
  ...
  conf->m_cpool = ngx_zmq_init_pool(zmq_init(1), cf->pool, ZMQ_REQ);
  return conf;
}
...
static int ngx_http_zmq_min(const int a, const int b)
{
  return a < b? a : b;
}
...
ngx_http_zmq_handler(ngx_http_request_t *r)
{
  ...
  ngx_int_t mlen = 0;
  int zrc = 0;
  int stype = ngx_http_zmq_get_socktype(r);
  ...
  ngx_zmq_set_endpt(zmq_config->m_cpool, zmq_config->zmq_endpoint);
  ngx_zmq_set_socktype(zmq_config->m_cpool, stype);
...
/* --------------- BEGIN MAGIC ------------------------- */
  ...
  {
    ngx_memcpy(it, c1->buf->pos, ngx_http_zmq_min((c1->buf->last - c1->buf->pos), r->headers_in.content_length_n - cpyd));
    it += ngx_http_zmq_min((c1->buf->last - c1->buf->pos), r->headers_in.content_length_n - cpyd);
    cpyd += ngx_http_zmq_min((c1->buf->last - c1->buf->pos), r->headers_in.content_length_n - cpyd);
  }
  
  ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "ngx_zmq sending %s to %s", (char*)input, zmq_config->m_cpool->m_endpt);
  /* ----------- ZMQ LOOPY THING -------------- */
  ngx_zmq_conn* con = ngx_zmq_get_conn(zmq_config->m_cpool);
  ...
  } while (ngx_http_zmq_to_ms(clock() - start) < to && zrc == -1 && zmq_errno() == EAGAIN);
  ...
  {
    ngx_zmq_rel_conn(zmq_config->m_cpool, &con);
